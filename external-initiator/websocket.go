...
func (con wsConn) Subscribe(message string) {
	...
	go func() {
		...
		for {
			...
			var requesterAddr, agg_x, dataset_code, selected_band, image_scale, start_date, end_date, event string

			txRes.convert(&requesterAddr, &agg_x, &dataset_code, &selected_band, &image_scale, &start_date, &end_date, &event)

			if event == "create_data" {

				fmt.Print("hereeeee.\n")
				//fmt.Print(string(message))
				startJob(requesterAddr, agg_x, dataset_code, selected_band, image_scale, start_date, end_date)
			}
		}
	}()
